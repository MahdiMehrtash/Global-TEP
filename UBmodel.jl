function get_UB_AC_OPF_model(bounds, XL, Qi)
...
sum(Loaddata[j,3] for j in 1:Num_load if Loaddata[j,1]==i)-Bsh[i]*Vi[i]^2-Qi[i]==0)   
...
if termination_status(model) == MOI.OPTIMAL || termination_status(model) == MOI.LOCALLY_SOLVED
    return objective_value(model)+(1/100)*sum(Qi[i] for i in 1:Num_bus)
else
    return 1e20, model
end
